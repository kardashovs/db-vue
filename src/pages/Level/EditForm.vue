...
<script>
    ...
    export default {
        ...
        created() {
            this.showLevel()
            this.title = (this.$route.params.id_level? ' Редактирование':'Доабвление') + ' уровня'
            this.$emit('setTitle', this.title)
        },
        methods: {
            addLevel() {
                this.$http.post('http://95.213.251.172:3000/languages/levels/'+this.$route.params.id, this.level , {
                    headers: {
                        'Content-Type': 'application/json'
                    }})
                    .then(response => {
                        this.$router.push({name: 'levels', params: {id: this.$route.params.id}})
                    })
            },
            updateLevel() {
